...
var json_url = base_server + 'jsondata/';
...
module.exports = {
  ...
  check_IP: function(ip, cb) {
    if (client_ipv4.isValid(client_ip)) {
      return cb(ip); //IPv4
    } else if (ipaddr.IPv6.isValid(client_ip)) {
      var ipv6 = ipaddr.IPv6.parse(client_ip);
      if (ipv6.isIPv4MappedAddress()) {
        return cb(ipv6.toIPv4Address().toString()) //IPv4
      } else {
        return cb(ipv6); //IPv6
      }
    } else {
  ...
  get_masternodelist: function(cb) {
    var uri = json_url + 'masternode_full_list.json';
    request({uri: uri, json: true}, function (error, response, body) {
      return cb(body);
    });
  },

  get_storadelist: function(cb) {
    var uri = json_url + 'storade_list.json';
    request({uri: uri, json: true}, function (error, response, body) {
      return cb(body);
    });
  },
