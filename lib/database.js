...
module.exports = {
  ...
  get_last_blocks: function(count, cb) {
    Tx.aggregate([{"$group" : {_id:"$blockindex", count:{ $sum: 1 }, total:{ $sum: "$total" }}}]).cursor({ batchSize: 1000 }).sort({blockindex: 'desc'}).limit(count).exec(function(err, blocks){
      if (err) {
        return cb(err);
      } else {
        return cb(blocks);
